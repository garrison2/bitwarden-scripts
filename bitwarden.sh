...
Options:
    ...
    --session <key>     load a session key (bypass login)
    --export, -E        export the session key to the shell (and do not logout)
    --no-load           do not check/load session key from the shell
"
    ...
    if [[ $autoload_session == "true" && ( ! -n $session ) && -n $BW_SESSION ]];
    then
        session=$BW_SESSION
    fi
